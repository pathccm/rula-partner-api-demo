...
export const api = {
  ...
  searchProviders(params: {
    ...
    limit?: number
    gender?: 'Agender' | 'Bigender' | 'Cis' | 'Female' | 'Gender fluid' | 'Male' | "My gender isn't listed" | 'Non-binary' | 'Prefer not to respond' | 'Trans'
    race?: 'American Indian or Alaska Native' | 'Asian' | 'Biracial or Multiracial' | 'Black or African American' | 'Caucasian or White' | 'Hispanic or Latinx' | 'Middle Eastern' | 'Native Hawaiian or Other Pacific Islander' | 'Other' | 'Prefer Not to Respond' | 'South East Asian'
    specialization?: 'Addiction' | 'ADHD' | 'Adoption' | 'Aging' | 'Alcohol Use' | "Alzheimer's" | 'Anger Management' | 'Anxiety' | "Autism or Asperger's Syndrome" | 'Behavioral Issues' | 'Bipolar Disorder' | 'Borderline Personality' | 'Career Counseling' | 'Child Abuse or Neglect' | 'Chronic Illness' | 'Chronic Pain' | 'Codependency' | 'Coping Skills' | 'Cultural Stress' | 'Depression' | 'Dissociative Disorders' | 'Divorce' | 'Domestic Violence' | 'Drug Abuse' | 'Eating Disorders' | 'Family Conflict' | 'Gambling' | 'Gender Identity' | 'Grief' | 'LGBTQIA+' | 'Life Transitions' | 'Pregnancy/Prenatal/Postpartum' | 'Relationship Issues' | 'Self Esteem' | 'Sleep or Insomnia' | 'Stress' | 'Trauma and PTSD'
  }) {
